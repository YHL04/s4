...
from jax.numpy.linalg import inv, matrix_power, eigh

from s4 import causal_convolution, scan_SSM
...
def log_step_initializer(dt_min=0.001, dt_max=0.1):
    """returns a uniform distribution with interval (np.log(dt_min), np.log(dt_max))"""
    def init(key, shape):
...
class S4Layer(nn.Module):
    ...
    # Special parameters with multiplicative factor on lr and no weight decay
    # (handled by main train script)
    lr = {
        "Lambda_re": 0.1,
        "Lambda_im": 0.1,
        "P": 0.1,
